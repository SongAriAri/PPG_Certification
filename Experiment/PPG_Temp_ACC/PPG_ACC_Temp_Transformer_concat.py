...
print(f"🚀 [Start] Hybrid Transformer + Concat Fusion + Visualization Ready", flush=True)
...
# ==========================================
# 3. 모델 아키텍처: Hybrid Transformer (NEW)
# ==========================================
class HybridTransformerEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # Transformer 입력을 위해 차원 변경: (Batch, Reduced_Len, d_model)
        x = x.permute(0, 2, 1)
        ...
        # 입력 길이가 다를 수 있는 경우를 대비해 슬라이싱하여 더함
        ...
        # 결과 분리
        # 1. Global Feature: CLS 토큰 (인증 분류용) -> (Batch, d_model)
        z_global = out[:, 0, :]
        
        # 2. Sequence Feature: 나머지 토큰들 (Fusion Attention용) -> (Batch, Reduced_Len, d_model)
        #    Cross-Attention에서 사용하기 위해 (Batch, d_model, Reduced_Len)으로 다시 transpose
...
class TriModalFusion(nn.Module):
    def __init__(self, num_users=16, d_model=256, num_heads=8):
        ...
        # [변경] ResNet 대신 HybridTransformer 사용
    ...
    def forward(self, x_ppg, x_temp, x_acc):
        ...
        # Attention (Key/Value는 Transpose 필요: B, d_model, L -> B, L, d_model)
...
def visualize_gradcam_samples(model, data_loader, device, save_path='gradcam_analysis.png', num_samples=3):
    ...
    # [수정] Transformer 구조에서는 'Stem'의 마지막 레이어를 타겟으로 삼는 것이 시각화에 유리함
    # Stem 출력: (B, d_model, Reduced_Len) 형태라 기존 GradCAM 로직과 호환됨
    target_layers = {
        'PPG': model.ppg_encoder.stem[-1],  # 마지막 ReLU
